        ...
        try{
            ...
            if($update){
                ...
                for($i = 0; $i < $pending_invoices->count(); $i++){
                    ...
                    // dd($pen_invoice);
                    $total_pending1 = $total_pending - $pen_invoice->pending_amount;
                    $total_pending = abs($total_pending1) - $pen_invoice->pending_amount;
                    // dd(abs($total_pending));
                    $total_pending = abs($total_pending) <= 0 ? 0 : abs($total_pending);
