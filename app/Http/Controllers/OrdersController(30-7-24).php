...
class OrdersController extends Controller
{
    ...
    public function createOrders(Request $request){
        try {
            ...
            $arr = $products->map(function ($product, $index) use ($data, $quantities, $delivery_dates, $product_discounts) {
                ...
                if($productData->category_id != '1' && $productData->category_id != NULL){
                    $update = Product::whereNull('deleted_at')->where('category_id', $productData->category_id)->decrement('quantity', $quantities[$index]);
                }else{
                    $arr1 = [
    ...
    public function updateOrders(Request $request)
    {
        try {
            ...
            if($productData->category_id != '1' && $productData->category_id != NULL){
                if($orderData->quantity > $request->quantity){
                    $update = Product::whereNull('deleted_at')->where('category_id', $productData->category_id)->increment('quantity', $quantities[$index]);
                }else{
                    $update = Product::whereNull('deleted_at')->where('category_id', $productData->category_id)->decrement('quantity', $quantities[$index]);
                }
            }else{
                if($orderData->quantity > $request->quantity){
                    $pro_qty = $orderData->quantity - $request->quantity;
                    $upPro_qty = $productData->quantity + $pro_qty;
                }else{
                    $pro_qty = $request->quantity - $orderData->quantity;
                    $upPro_qty = $productData->quantity - $pro_qty;
                }
                $arr1 = [
                    'quantity' => $upPro_qty
                ];
                $update = Product::where('id', $request->product)->update($arr1);
            }
            if(!$update){
                return response()->json(['status' => 400, 'message' => 'Quantity Reduction Failed.']);
            }
