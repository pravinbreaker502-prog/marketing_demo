...
class OrdersController extends Controller
{
        ...
        try {
            ...
            $arr = $products->map(function ($product, $index) use ($data, $quantities, $delivery_dates, $product_discounts) {
                ...
                if ($product_discounts[$index] == '0' || $product_discounts[$index] == null) {
                    ...
                    $ttl_sellpri = $product_sellprice*$quantities[$index];
    ...
    public function updateOrders(Request $request)
    {
        try {
            ...
            if($categories && !in_array('1', $categories)){
                ...
                $proq_up = Product::find($request->product);
                $qty_check = $proq_up->quantity - $request->quantity;
                // dd($qty_check);
                if((int)$qty_check < 0){
                ...
                if($update1){
                    ...
                    $update = Product::where('id', $request->product)->update(['quantity' => (int)$proq_up->quantity-(int)$request->quantity, 'updated_at' => now()]);
                }else{
                    return response()->json(['status' => 400, 'message' => 'Quantity increment Failed.']);
                }
            ...
            if ($request->product_discount == '0' || $request->product_discount == null) {
                $discount_per = $product_dis;
                $ttl_sellpri = $product_sellprice*$request->quantity;
                $dis_amount = ($product_dis / 100) * $ttl_sellpri;
            } else {
    ...
    public function deleteOrders(Request $request){
        try {
            ...
            $get = Orders::where('id', $request->id)->first();
            // dd(['deleted_at' => now()]);
    ...
    {
        try {
            // Fetch the orders with related customer and product data
            $orders = Orders::select('orders.*', 'customers.company_name', 'customers.client_address', 'customers.client_mobile', 'employees.employee_name', 'employees.employee_type', 'employees.employee_address', 'employees.employee_mobile', 'products.product', 'products.actual_price as product_price')
                ->leftJoin('customers', 'customers.id', '=', 'orders.customer_id')
                ->leftJoin('employees', 'employees.id', '=', 'orders.employee_id')
                ->leftJoin('products', 'products.id', '=', 'orders.product_id')
                ->whereNull('orders.deleted_at')
            ...
            $html = view('marketing.Orders.test_invoice', compact('orders', 'invoice_no'))->render();
    
            // You can create an mPDF instance if needed for additional formatting or rendering checks
            $mpdf = new Mpdf();
            $mpdf->WriteHTML($html);
    
            // Instead of generating a PDF, return the raw HTML for preview
            return response()->json(['status' => 200, 'preview_html' => $html]);
    ...
    public function generateInvoice(Request $request)
    {
        try {
            ...
            if (!$customer) {
                return response()->json(['status' => 400, 'message' => 'Please select a particular customer to proceed with the invoice.']);
            }
            ...
            if ($invoice) {
                return response()->json(['status' => 400, 'message' => 'Issue in generating invoice. Please contact admin.']);
            }
            ...
            $html = view('marketing.Orders.test_invoice', compact('customer', 'orders', 'invoice_no'))->render();
            ...
            // Output the PDF as a string (to save to file)
            ...
            if ($invoice_insert) {
                // Update the orders with the invoice ID and number
                $order_update = Orders::whereIn('id', $request->order_ids)->update([
