...
class ProductController extends Controller
{
    ...
    public function getProducts(Request $request)
    {
        ...
        return DataTables::of($products)
            ...
            ->addColumn('product_caregory', function ($products) {
                if($products->category_id != NULL || $products->category_id != ''){
                    $pro_cat = DB::table('products_categories')->whereIn('id', json_decode($products->category_id))->pluck('category_name');
                    return '<p class="f-light">'. implode(", ", $pro_cat->toArray()) .'</p>';
                }else{
                    return '<p class="f-light">-</p>';
                }
            })
            ...
            ->addColumn('qty', function ($products) {
                $categories = json_decode($products->category_id, true);
    ...
    public function createProducts(Request $request)
    {
        try {
            ...
            $messages = [
                ...
                'product_standard.required' => 'The standard is required.',
            ...
            if(!in_array('1', $request->product_category)){
                $pro_low_qty = Product::whereNull('deleted_at')->whereRaw('JSON_CONTAINS(category_id,  \'["'.$request->product_category[0].'"]\')')->whereRaw('JSON_LENGTH(category_id) > 1')->orderBy('quantity', 'ASC')->limit(1)->pluck('quantity');
                $quantity_fin = $pro_low_qty[0] ?? 0;
            }else{
                $quantity_fin = $request->product_qty;
            }
            
            $arr = [
                'standard' => $request->product_standard,
                'category_id' => json_encode($request->product_category, true),
                'product' => $request->product_name,
                'page_slug' => $product_slug,
                'quantity' => $quantity_fin,
    ...
    public function ShowOrHideQuantity(Request $request){
        if(!in_array('1', $request->ids)){
            $pro_low_qty = Product::whereNull('deleted_at')->whereRaw('JSON_CONTAINS(category_id,  \'["'.$request->ids[0].'"]\')')->orderBy('quantity', 'ASC')->limit(1)->pluck('quantity');
            // dd($pro_low_qty);
            return response()->json(['status' => 200, 'qty' => $pro_low_qty[0], 'type' => 'hide']);
        }else{
            return response()->json(['status' => 200, 'type' => 'show']);
        }
    }
