...
class verifierPendingController extends Controller
{
    public function index()
    {
        ...
        return view('verifier.pending-orders.verifier-pending-orders', compact('datas'));
    }
    
    public function getVerifierPendingOrders(Request $request)
    {
    ...
    public function updateVerifierPendingOrders(Request $request){
        try {
            $rules = [
                    'id' => ['required']
                ];
            ...
            // dd(['deleted_at' => now()]);
            $update = Orders::where('id', $request->id)->update(['status' => 'Verified', 'updated_at' => now()]);
            if($update){
