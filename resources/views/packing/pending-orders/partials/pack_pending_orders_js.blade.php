<script>
    $(function(){
        ...
        var table = new DataTable('#orders-table', {
            ajax: {
                url: "{{ route('pack-pending-orders.get') }}", // Use Laravel's route helper to generate the URL
                type: 'POST',
                data: function(d){
                    d._token = '{{ csrf_token() }}'
                }
    ...
    function ChangeTheOrderStatus(id){
        ...
        Swal.fire({
                title: 'Are you sure?',
                text: "Are you packed the order?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Yes, Packed!',
                cancelButtonText: 'No, cancel!',
                reverseButtons: true
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('pack-pending-orders-status.update') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: id
