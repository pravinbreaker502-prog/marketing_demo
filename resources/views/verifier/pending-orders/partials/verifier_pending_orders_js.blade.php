<script>
    $(function(){
        ...
        var table = new DataTable('#orders-table', {
            ajax: {
                url: "{{ route('verifier-pending-orders.get') }}", // Use Laravel's route helper to generate the URL
                type: 'POST',
                data: function(d){
                    d._token = '{{ csrf_token() }}'
                }
    ...
    function ChangeTheOrderStatus(id){
        ...
        Swal.fire({
                title: 'Are you sure?',
                text: "Are you Verified the order?",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('verifier-pending-orders.update') }}",
                        type: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            id: id
